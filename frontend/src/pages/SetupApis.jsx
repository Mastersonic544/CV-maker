...
const KEY_META = {
  OPENROUTER_API_KEY: {
    hint: 'Powers all AI features — CV generation, persona research, cover letters. Get your key from OpenRouter.',
    url: 'https://openrouter.ai/keys',
    required: true,
    section: 'AI — OpenRouter',
  },
  LINKEDIN_EMAIL: {
    hint: 'Your LinkedIn login email — used to scrape job listings.',
    url: 'https://linkedin.com',
    required: false,
    section: 'LinkedIn — Job Scraping',
  },
  LINKEDIN_PASSWORD: {
    hint: 'Stored encrypted locally. Used alongside your email for LinkedIn scraping.',
    url: 'https://linkedin.com',
    required: false,
    section: 'LinkedIn — Job Scraping',
  },
  SMTP_USER: {
    hint: 'Your Gmail address — e.g. you@gmail.com. This is what appears in the From field. Emails are sent directly from your account.',
    url: 'https://mail.google.com',
    required: false,
    section: 'Email — Gmail SMTP',
  },
  SMTP_PASSWORD: {
    hint: 'A Gmail App Password — NOT your Gmail login password. Create one at: Google Account → Security → 2-Step Verification → App Passwords. Name it anything, copy the 16-char code.',
    url: 'https://myaccount.google.com/apppasswords',
    required: false,
    section: 'Email — Gmail SMTP',
  },
  SENDER_NAME: {
    hint: 'Your full name shown in the From field — e.g. "Yassine Dhouib".',
    url: null,
    required: false,
    section: 'Email — Gmail SMTP',
  },
...
const KeyCard = ({ k, value, visible, onChange, onToggleShow }) => {
  ...
  return (
    ...
    >
      ...
      <div className="relative">
        <input
          type={visible ? 'text' : (isPassword ? 'password' : 'text')}
          value={value}
          ...
          {(() => {
            ...
            keys.forEach(k => {
              ...
              if (section !== lastSection) {
                if (currentGroup.length) sections.push({ label: lastSection, items: currentGroup });
                currentGroup = [k];
                lastSection = section;
              } else {
                currentGroup.push(k);
            ...
            return sections.map(({ label, items }) => (
              <div key={label}>
                <div className="flex items-center gap-3 mb-3">
                  <span style={{ width: 14, height: 1, background: 'var(--cv-cyan)' }} />
                  <span className="font-dm" style={{ fontSize: '0.55rem', letterSpacing: '0.22em', textTransform: 'uppercase', color: 'var(--cv-cyan)' }}>
                    {label}
                  </span>
                </div>
                <div className="space-y-3">
                  {items.map(k => (
                    <KeyCard
                      ...
                      k={k}
                      value={values[k.key] || ''}
                      visible={show[k.key] || false}
                      onChange={v => setValues(prev => ({ ...prev, [k.key]: v }))}
                      onToggleShow={() => setShow(prev => ({ ...prev, [k.key]: !prev[k.key] }))}
                    />
                  ))}
                </div>
              </div>
